...
const Creches = () => {
  ...
  const [leftStatut, setLeftStatut] = useState("ouvert"); // "ouvert" | "ferme" | "tous"
  ...
  const handleCreateCreche = async () => {
    ...
    try {
      await axios.post("/api/creches", { nom: newNom, directeur: newDirectrice }, { withCredentials: true });
  ...
  const handleRename = async () => {
    ...
    try {
      await axios.put("/api/creches/rename", { ancienNom: sel[0].nom, nouveauNom: editValue }, { withCredentials: true });
  ...
  const handleChangeDirecteur = async () => {
    ...
    try {
      await axios.put("/api/creches/change-directeur", { nom: sel[0].nom, directeur: editValue2 }, { withCredentials: true });
  ...
  const handleFermer = async () => {
    ...
    try {
      for (const c of getSelectedCreches()) {
        await axios.put("/api/creches/fermer", { nom: c.nom }, { withCredentials: true });
      }
      fetchCreches();
    } catch (e) { alert(e.response?.data || "Erreur"); }
  };
  ...
  const handleRouvrir = async () => {
    ...
    try {
      for (const c of getSelectedCreches()) {
        await axios.put("/api/creches/rouvrir", { nom: c.nom }, { withCredentials: true });
      }
      fetchCreches();
    } catch (e) { alert(e.response?.data || "Erreur"); }
  };
  ...
  const handleSupprimer = async () => {
    ...
    try {
      for (const c of getSelectedCreches()) {
        await axios.delete("/api/creches/delete", { data: { nom: c.nom }, withCredentials: true });
      }
      setSelectedNoms([]);
      fetchCreches();
    } catch (e) { alert(e.response?.data || "Erreur"); }
